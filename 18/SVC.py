...
random_s = int(input("seed:"))

# P.514 二項分類用データ生成
X, y = make_classification(
    n_samples=100, n_features=2, n_redundant=0, random_state=random_s)


# P.518 学習用と評価用にデータを分割
train_X, test_X, train_y, test_y = \
    train_test_split(X, y, random_state=random_s)
...
# 最後の表⽰前に以下を追加
output_X = np.concatenate([train_X, test_X]) # 出⼒⽤の全データ 学習⽤＋評価⽤ の順
output_y = np.concatenate([train_y, test_y]) # 出⼒⽤の全データ 学習⽤＋評価⽤ の順
scatter_kwargs = {'s': 100, 'edgecolor': None, 'alpha': 0.7} # 散布図⽤表⽰設定
scatter_highlight_kwargs = {'s': 100, 'label': 'Test data', 'alpha': 0.7} # 評価⽤表⽰設定
